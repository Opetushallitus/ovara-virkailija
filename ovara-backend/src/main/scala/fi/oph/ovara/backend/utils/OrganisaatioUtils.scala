...
object OrganisaatioUtils {
  def recursiveListParentsAndSelf(
      parentOid: String,
      orgs: Vector[OrganisaatioParentChild],
      foundParents: List[Organisaatio] = List()
  ): List[Organisaatio] = {
    if (orgs.isEmpty) {
      foundParents
    } else {
  ...
  def mapOrganisaationHakukohteetToParent(
      orgs: Vector[OrganisaatioParentChild],
      organisaatioWithHakukohteet: Map[Option[String], Vector[KoulutuksetToteutuksetHakukohteetResult]]
  ): Map[String, List[(Organisaatio, OrganisaationKoulutuksetToteutuksetHakukohteet)]] = {
    organisaatioWithHakukohteet
      .map(org => {
        ...
        orgs.find(o => o.organisaatio.organisaatio_oid == orgOid.getOrElse(false)) match {
          case Some(organisaatio) =>
            Some(
              (
                koulutustoimija.head,
                OrganisaationKoulutuksetToteutuksetHakukohteet(Some(organisaatio.organisaatio), org._2)
              )
            )
          case _ =>
            None
      ...
      .toList
      .flatten
      .groupBy(o => o._1.organisaatio_oid)
  }
}
