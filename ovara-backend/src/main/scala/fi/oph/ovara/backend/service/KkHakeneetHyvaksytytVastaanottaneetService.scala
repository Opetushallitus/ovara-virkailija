...
import fi.oph.ovara.backend.raportointi.dto.{
  buildKkHakeneetHyvaksytytVastaanottaneetParamsForExcel,
  ...
  def get(
    ...
    Try {
      ...
      val isOrganisaatioRajain =
        (koulutustoimija.isDefined || oppilaitokset.nonEmpty || toimipisteet.nonEmpty) && orgOidsForQuery.nonEmpty
      ...
      val sortedResult = queryResult.sortWith {
        case (
              a: KkHakeneetHyvaksytytVastaanottaneetHakukohteittain,
              b: KkHakeneetHyvaksytytVastaanottaneetHakukohteittain
            ) =>
        ...
        case (a: KkHakeneetHyvaksytytVastaanottaneetResult, b: KkHakeneetHyvaksytytVastaanottaneetResult) =>
          collator.compare(
            a.otsikko.getOrElse(Kieli.withName(asiointikieli), ""),
            b.otsikko.getOrElse(Kieli.withName(asiointikieli), "")
          ) < 0
      }
      val sumQueryResult =
        db.run(sumQuery, "kkHakeneetHyvaksytytVastaanottaneetRepository.selectHakijatYhteensaWithParams")
      val ensikertalaisetSumQueryResult =
        if (vainEnsikertalaiset)
          sumQueryResult
        else
          db.run(
            ensikertalaisetSumQuery,
            "kkHakeneetHyvaksytytVastaanottaneetRepository.selectEnsikertalaisetHakijatYhteensaWithParams"
          )
      val maksuvelvollisetSumQueryResult = db.run(
        maksuvelvollisetSumQuery,
        "kkHakeneetHyvaksytytVastaanottaneetRepository.selectMaksuvelvollisetHakijatYhteensaWithParams"
      )
