...
class ControllerSpec extends AnyFlatSpec with Matchers {
  ...
  "koulutukset_toteutukset_hakukohteet" should "log audit parameters correctly" in {
    ...
    val mockKkKoulutuksetToteutuksetHakukohteetService =
      mock(classOf[KorkeakouluKoulutuksetToteutuksetHakukohteetService])
    ...
    when(
      mockKoulutuksetToteutuksetHakukohteetService.get(
        any(),
        any(),
        any(),
        any(),
