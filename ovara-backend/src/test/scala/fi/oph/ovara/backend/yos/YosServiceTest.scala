...
import fi.oph.ovara.backend.yos.YosTestUtils.{HENKILO, OPISKELUOIKEUS, ORG_OID, VASTAANOTTO, YOS_VALINTAREKISTERI}
...
class YosServiceTest {
  ...
  def palauttaaPaattyvatOpiskeluoikeudet(): Unit = {
    ...
    when(db.run(any(), ArgumentMatchers.eq("valintarekisteriYosQuery"))).thenReturn(List(YOS_VALINTAREKISTERI))
    ...
    assertTrue(oikeus.naytettyHakijalle)
    assertEquals(Some(LocalDate.of(2026, 7, 15)), oikeus.uudenOpiskeluoikeudenAlkamispvm)
    assertEquals(Some(LocalDate.of(2026, 7, 14)), oikeus.opiskeluoikeudenPaattymispvm)
  }
  ...
  def palauttaaPaattyvatOpiskeluoikeudetIlmanValintarekisteinYosTietojakin(): Unit = {
    ...
    when(db.run(any(), ArgumentMatchers.eq("sitovastiVastaanottaneetQuery"))).thenReturn(List(VASTAANOTTO))
    when(db.run(any(), ArgumentMatchers.eq("yosHenkilotQuery"))).thenReturn(List(HENKILO))
    when(db.run(any(), ArgumentMatchers.eq("valintarekisteriYosQuery"))).thenReturn(List.empty)
    val oikeudet = service.getPaattyvatOpiskeluOikeudet(orgs, params)
    assertEquals(1, oikeudet.size)
    val oikeus = oikeudet.head
    assertEquals("Ruhtinas", oikeus.etunimet)
